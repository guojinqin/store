...
def findByAccount(account):
    sql = "select * from bank.information where account = %s;"
    param = [account]
...
def bank_addUser(username,password,country,province,street,door,money):
    ...
    # 插入数据
    sql2 = "insert into information value(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
def bank_saveMoney(ac,money):
    ...
    data = select(sql, param)
    if len(data) != 0:
        sql1 = "update bank.information set money = money + %s where account = %s;"
        param2 = [money, ac]
        update(sql1, param2)
        return True
    return False
...
def bank_selectUser(account, password):
    sql1 = "select * from information where account  = %s"
    param1 = [account]
    data1 = select(sql1, param1)
    if data1 != None and len(data1) != 0:
        sql2 = "select password from information where account = %s"
        param2 = [account]
...
def bank_takeMoney(account, password, money):
    ...
    if len(data1) != 0:
        ...
        data2 = select(sql2, param2)
        if str(data2[0][0]) == password:
            sql3 = "select money from information where account = %s"
            param3 = [account]
            data3 = select(sql3, param3)
            if data3[0][0] < money:
                return 3
            else:
                sql4 = "update information set money=money-%s where account = %s;"
